...
public class RentalController {
    ...
    @ApiResponses(
            value = { @ApiResponse(responseCode = "200", description = "Successfully fetched the list of rentals"),
    ...
    @Operation(summary = "Create new rental",
            description = "Create a new rental with the provided details such as "
                    + "name, surface, price, a picture and a description.")
    @ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Rental created successfully"),
            @ApiResponse(responseCode = "401", description = "Unauthorized, user not authenticated") })
    @PostMapping(value = "/rentals", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<Object> createRental(
            @Parameter(description = "Name of the rental", required = true,
                    example = "Apartment") @RequestParam("name") String name,

            @Parameter(description = "Surface area of the rental in square meters", required = true,
                    example = "50") @RequestParam("surface") @DecimalMin(value = "0", inclusive = true,
                            message = "Surface can not be a negative value") BigDecimal surface,

            @Parameter(description = "Price of the rental per night", required = true,
                    example = "200") @RequestParam("price") @DecimalMin(value = "0", inclusive = true,
                            message = "Price can not be a negative value") BigDecimal price,

            @Parameter(description = "Picture of the rental (picture file)",
                    required = true) @RequestParam("picture") MultipartFile picture,

            @Parameter(description = "Description of the rental", required = true,
                    example = "A nice apartment") @RequestParam("description") String description)
            throws IOException {
    ...
    @Operation(summary = "Update existing rental",
            description = "Update the details of an existing rental, such as "
                    + "name, surface, price, and description.")
    @ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Rental updated successfully"),
            @ApiResponse(responseCode = "401",
                    description = "Unauthorized, user not authorized to update the rental") })
    @PutMapping("/rentals/{id}")
    public ResponseEntity<Object> updateRental(
            @Parameter(description = "Unique ID of the rental to update", required = true,
                    example = "2") @PathVariable Integer id,

            @Parameter(description = "Updated name of the rental", example = "House") @RequestParam(value = "name",
                    required = false) String name,

            @Parameter(description = "Updated surface area of the rental in square meters",
                    example = "90") @RequestParam(value = "surface", required = false) @DecimalMin(value = "0",
                            inclusive = true, message = "Surface can not be a negative value") BigDecimal surface,

            @Parameter(description = "Updated price of the rental per night", example = "400") @RequestParam(
                    value = "price", required = false) @DecimalMin(value = "0", inclusive = true,
                            message = "Price can not be a negative value") BigDecimal price,

            @Parameter(description = "Updated description",
                    example = "A nice house") @RequestParam(value = "description", required = false) String description)
            throws IOException {
    ...
    @Operation(summary = "Get rental by ID",
            description = "Retrieve the details of a specific rental based on its unique ID.")
    @ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Successfully fetched the rental data"),
            @ApiResponse(responseCode = "401", description = "Unauthorized, user not authenticated") })
    @GetMapping("/rentals/{id}")
    public ResponseEntity<RentalDTO> getRentalById(@Parameter(description = "Unique ID of the rental to retrieve",
